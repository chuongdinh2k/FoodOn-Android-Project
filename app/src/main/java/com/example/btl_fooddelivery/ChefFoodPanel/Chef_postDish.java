...
//        databaseReference = firebaseDatabase.getInstance().getReference("FoodSupplyDetails")
    ...
    private void uploadImage(){
        ...
        DatabaseReference myRef = database.getReference("FoodSupplyDetails");
        if(imageuri != null){
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                    ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            Toast.makeText(Chef_postDish.this, RandomUId, Toast.LENGTH_SHORT).show();
                            FoodSupplyDetails info =  new FoodSupplyDetails(dishes,quantity,price,description,String.valueOf(uri),RandomUId,ChefId);
//                            firebaseDatabase.getReference("FoodSupplyDetails").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUId)
...
//                                }
//                            });
                            myRef.child(city).child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(RandomUId).setValue(info).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    progressDialog.dismiss();
                                    Toast.makeText(Chef_postDish.this, "Dish posted successfully", Toast.LENGTH_SHORT).show();
